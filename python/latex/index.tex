...
The first section starts from the comment {\ttfamily \# Check p\+\_\+system of PARAMETERS namelist}. This section checks for the presence of the PARAMETERS namelist, and populates it if it is found to be empty. A `elif p\+\_\+system==\textquotesingle{}your\+\_\+system'\+:{\ttfamily section will need to be added to the end here that automatically populates the PARAMETERS namelist with the values you just defined in}\mbox{\hyperlink{namespace_dice_py_1_1dice__defaults}{Dice\+Py.\+dice\+\_\+defaults}}\`{}.
...
\begin{DoxyItemize}
\item {\ttfamily isvalid(tag, values, options)}, which checks if a given {\ttfamily tag}\textquotesingle{}s {\ttfamily values} match one of the expected {\ttfamily options} from a predefined list.
\item {\ttfamily check\+\_\+float(tag, tags, params, defaults, idx=None)}, which checks if a given {\ttfamily tag}\textquotesingle{}s value(s) are floats, and replaces them with the {\ttfamily tag}\textquotesingle{}s default values if not.
\item {\ttfamily check\+\_\+grid(tag, tags, params, default\+\_\+grid)}, which does the same as above but for a grid-\/type parameter, ie. a {\ttfamily (start, stop, num)} set of numbers to run a linear spacing function on, similar to numpy\textquotesingle{}s {\ttfamily linspace}.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item {\ttfamily isint(s)} and {\ttfamily isfloat(s)}, which check if the string {\ttfamily s} that they are passed is an integer or a float, respectively. These are used consistently throughout the writer to ensure that the user is providing the correct kind of input and not, for example, passing a float to the variable for the total number of VQMC steps, which should always be an integer.
\item {\ttfamily query\+\_\+default(param, default)}, which queries if a user wishes to use the {\ttfamily default} value for a {\ttfamily param}, as described in {\ttfamily \mbox{\hyperlink{namespace_dice_py_1_1dice__defaults}{Dice\+Py.\+dice\+\_\+defaults}}}.
\item {\ttfamily query\+\_\+step(prompt, tag, defaults, idx)}, which asks the user for a positive integer ie. a \textquotesingle{}step\textquotesingle{} value, and {\ttfamily query\+\_\+posfloat(prompt, tag, defaults, idx)}, which does the same for positive floats.
\item {\ttfamily query\+\_\+grid(param)}, which constructs a {\ttfamily (start, stop, num)} grid to be linearly spaced once in the Fortran backend, while checking that values in the grid are of the correct type to avoid errors.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item {\ttfamily QHOEnergy\+Plot(results\+\_\+file, sliceby=(None, None))} reads {\ttfamily results\+\_\+file} and plots the energies against variational alpha values specified in the problem, as well as the uncertainties in the energies. The sliceby option allows one to plot only a section of the graph by typing the indices of the slice you are interested in.
\item {\ttfamily QHOWfn\+Plot(chain\+\_\+file, show\+\_\+analytic=False)} reads a file containing the Markov chains and plots the distribution of the QHO wave functions, also showing the energies of these positions as well. It can also plots an analytical solution for the wave function as well, if desired.
\end{DoxyItemize}
...
This documentation was generated from commit 369e552 