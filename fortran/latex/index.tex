...
The first step is to add the derived type of the new problem to {\ttfamily \mbox{\hyperlink{namespaceshared__data}{shared\+\_\+data}}}, under the section \char`\"{}\+Derived Types\char`\"{}. Below is a sample format of how the derived type should look like.
...
\end{DoxyCode}
 If the user wants to add any other numerical constants then they can be added to the {\ttfamily constants} module.

The next step is to modify {\ttfamily solvers}. Regardless of the new problem the user will need to implement a function that calculates the transition probability of the current step and the proposed step. Below is a sample format of how the transition probability function should look like.
...
The next step is to add the automatic gradient descent-\/based search (GDBS) to {\ttfamily vqmc}. The user should use \char`\"{}\+Auto\+\_\+\+H\+\_\+2\char`\"{}, \char`\"{}\+Auto\+\_\+\+H\+\_\+2\+\_\+plus\char`\"{} sub-\/routines as a template and should only modify the VQMC solver being called (see below). This modification should be place after the new VQMC solver.
...
\end{DoxyCode}
 If the user does not want to do a GDBS then they can ignore the previous step. Instead they can add a grid search to {\ttfamily vqmc}. The user should use \char`\"{}\+Grid\+\_\+\+H\+\_\+2\char`\"{}, \char`\"{}\+Grid\+\_\+\+H\+\_\+2\+\_\+plus\char`\"{} sub-\/routines as a template and should only modify the VQMC solver being called (see below). This sub-\/routine should be placed after the new VQMC solver.
...
\end{DoxyCode}
 The user can of course add both of the search methods mentioned above, but it is important that they at least add one. The final item the user has to add to {\ttfamily vqmc} is the equilibration run subroutine. This can be based off \char`\"{}\+Equil\+\_\+\+QHO\char`\"{} or \char`\"{}\+Equil\+\_\+\+H\+\_\+2\char`\"{} and the user should only modify the VQMC solver being used like in the previous step. ~\newline
