...
The first step is to add the derived type of the new problem to the file /src/shared\+\_\+data.f90, under the section \char`\"{}\+Derived Types\char`\"{}. Below is a sample format of how the derived type should look like.
...
\DoxyCodeLine{        \textcolor{keywordtype}{integer(int32)} :: steps=100000\_int32         }
\DoxyCodeLine{        \textcolor{keywordtype}{integer(int32)} :: burn\_step=1000\_int32       }
\DoxyCodeLine{        \textcolor{keywordtype}{integer(int32)} :: thin\_step=10\_int32         }
\DoxyCodeLine{\textcolor{keywordtype}{        real}(real64) :: rcut=3.0\_real64              }
\DoxyCodeLine{\textcolor{keywordtype}{        real}(real64) :: sigma=1.8\_real64             }
\DoxyCodeLine{        \textcolor{keywordtype}{logical} :: auto\_params = .true.              }
\DoxyCodeLine{\textcolor{keywordtype}{        real}(real64) :: parameter\_for\_new\_problem = 0.4\_real64      }
\DoxyCodeLine{\textcolor{keywordtype}{        real}(real64), \textcolor{keywordtype}{dimension(3)} :: parameter\_grid = (/0,1,10/)}
\DoxyCodeLine{\textcolor{keywordtype}{        real}(real64), \textcolor{keywordtype}{dimension(3)} :: bond = (/1,3,10/)}
\DoxyCodeLine{    \textcolor{keyword}{end type }H2\_type}

\end{DoxyCode}
 If the user wants to add are any other constants then they can be added to the \char`\"{}constants\char`\"{} module in /src/shared\+\_\+data.f90.

The next step is to modify the file /src/quantum\+\_\+solvers.f90. Regardless of the new problem the user will need to implement a function that calculates the transitional probability of the current step and the proposed step. Below is a sample format of how the transitional probability function should look like.
...
\end{DoxyCode}
 If the user uses one of the \char`\"{}\+VQMC\+\_\+\+QHO\char`\"{}, \char`\"{}\+VQMC\+\_\+\+H\+\_\+2\+\_\+plus\char`\"{} or \char`\"{}\+VQMC\+\_\+\+H\+\_\+2\char`\"{} sub-\/routines as a template then the burning and thinning feautures can be reused for the new problem. It is important to stick to one of the VQMC sub-\/routines so that the outputs are the same.

The next step is to add the automatic gradient descent based search (GDBS) to the file /src/vqmc.f90. The user should use \char`\"{}\+Auto\+\_\+\+H\+\_\+2\char`\"{}, \char`\"{}\+Auto\+\_\+\+H\+\_\+2\+\_\+plus\char`\"{} sub-\/routines as a template and should only modify the VQMC solver being called (see below). This modification should be place after the new VQMC solver.
...
\end{DoxyCode}
 If the user does not want to do a GDBS then they can ignore the previous step. Instead they can add a grid search to the file /src/vqmc.f90. The user should use \char`\"{}\+Grid\+\_\+\+H\+\_\+2\char`\"{}, \char`\"{}\+Grid\+\_\+\+H\+\_\+2\+\_\+plus\char`\"{} sub-\/routines as a template and should only modify the VQMC solver being called (see below). This sub-\/routine should be placed after the new VQMC solver.
